...
module clk_divider (
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            counter <= 32'd0;
            was_reset <= 1;
            prev_clk_bit_select <= clk_bit_select; // also reset prev selection
        end else if (was_reset) begin
            counter <= clk_bit_select - 1;
            was_reset <= 0;
            prev_clk_bit_select <= clk_bit_select;
        end else if (clk_bit_select != prev_clk_bit_select) begin
            // Reset counter if clk_bit_select changed
            counter <= 32'd0;
            prev_clk_bit_select <= clk_bit_select; 
